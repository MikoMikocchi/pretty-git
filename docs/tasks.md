...
- [ ] Производительность (baseline)
  - [x] Инфраструктура бенчей: `rake perf:baseline`, `scripts/perf_baseline.rb`, `docs/perf.md`
  - [x] Прогон на крупном репозитории (`~/TypeScript-main`), результаты добавлены в `docs/perf.md`
...
- [ ] Совместимость и надёжность
  ...
  - [x] Smoke на Windows (best‑effort)
  - [x] Детерминизм: инвариантные тесты для YAML/XML рендереров (стабильный вывод при разном порядке входа)
- [ ] UX CLI
  - [x] Предупреждения при `--theme`/`--no-color` с не‑консольными форматами
  - [x] Улучшить `--help` (структура, примеры), консистентные тексты ошибок
  - [x] Базовое автодополнение (скрипт генерации; подготовка, если решим включать)
  - [x] Обновление bash/zsh автодополнений: `-f`/`-o`/`-l`, значения `--time-bucket` (day|week|month), 2‑й позиционный — путь к репозиторию
- [ ] CI/Release hygiene
  - [x] Smoke для Homebrew formula (установка и простой отчёт)
  - [x] Проверки стабильности снапшотов (golden files) для ключевых отчётов
  - [x] Документация тестирования: добавлен `docs/testing.md`, ссылки из `README.md`/`README.ru.md`/`CONTRIBUTING.md`
  - [x] Обновление `CHANGELOG.md` (`[Unreleased]`): тестирование, инварианты детерминизма, shell completions
