...
RSpec.describe PrettyGit::Render::XmlRenderer do
  ...
  it 'is deterministic for hotspots regardless of input item order' do
    ...
    xa = render_xml('hotspots', a)
    xb = render_xml('hotspots', b)
    expect(xa).to eq(xb)
  end
  ...
  it 'is deterministic for churn regardless of input item order' do
    ...
    xa = render_xml('churn', a)
    xb = render_xml('churn', b)
    expect(xa).to eq(xb)
  end
  ...
  it 'is deterministic for ownership regardless of input item order' do
    ...
    xa = render_xml('ownership', a)
    xb = render_xml('ownership', b)
    expect(xa).to eq(xb)
  end
end
